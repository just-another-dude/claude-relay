...
class TmuxSession:
    ...
    def create(self, workspace: str = Config.WORKSPACE) -> bool:
        ...
        # Create session
        result = subprocess.run(
            ['tmux', 'new-session', '-d', '-s', self.session, '-c', workspace],
            capture_output=True
        )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        claude_running = any(x in output.lower() for x in [
            'claude', '>', 'thinking', 'working', 'reading', 'writing'
        ])
        
        # Check for pending approval
        needs_approval = any(x in output.lower() for x in [
            '[1]', '[2]', 'approve', 'reject', 'y/n', 'yes/no', 'continue?'
        ])
...
class ClaudeCodeBridge:
    ...
    def _extract_response(self, before: str, after: str, prompt: str) -> str:
        ...
        new_lines = [l for l in lines_after if l not in lines_before]
        
        # Filter out the prompt itself
        new_lines = [l for l in new_lines if prompt[:30] not in l]
    ...
    def stop(self) -> str:
        ...
        # Send Ctrl+C
        subprocess.run(
            ['tmux', 'send-keys', '-t', self.tmux.pane, 'C-c'],
            capture_output=True
        )
